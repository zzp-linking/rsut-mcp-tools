...
pub fn manage_services(args: serde_json::Value, manager: &ServiceManager) -> String {
    let args: ServiceToolArgs = match serde_json::from_value(args) {
        Ok(a) => a,
    ...
    match args.action.to_lowercase().as_str() {
        "list" => {
            ...
            let query_type = if only_running {
                "MCP_LIST_RUNNING"
            } else {
                "MCP_LIST_ALL"
            };
