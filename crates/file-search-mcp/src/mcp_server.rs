...
pub fn run_loop(config: Config) {
    ...
    for line in stdin.lock().lines() {
        ...
        let response = handle(method, params, id, &config);
...
fn handle(method: &str, params: Option<Value>, id: Value, config: &Config) -> Option<Value> {
    match method {
        ...
        "tools/call" => {
            ...
            let result = tools::call_tool(tool_name, args, config);
